...
calib_button = tk.Button(ctrl_frame, text="Auto Calibrate", width=15)
calib_button.grid(row=0, column=4, padx=5)
...
def on_close():
    global running, stream, ani
    running = False  # stop audio callback
    if stream:
        stream.stop()
        stream.close()
    if ani:
        ani.event_source.stop()  # stop the FuncAnimation
    root.quit()   # exit Tkinter mainloop

root.protocol("WM_DELETE_WINDOW", on_close)

def open_calib_window():
    if not running:
        print("Start the stream first.")
        return
    calib_win = tk.Toplevel(root)
    calib_win.title("Calibration")
    tk.Label(calib_win, text="Known Peak Voltage (V):").pack(pady=5)
    volt_entry = tk.Entry(calib_win)
    volt_entry.pack()
    tk.Label(calib_win, text="Known Frequency (Hz):").pack(pady=5)
    freq_entry = tk.Entry(calib_win)
    freq_entry.pack()
    progress = ttk.Progressbar(calib_win, orient="horizontal", length=200, mode="determinate", maximum=100)
    progress.pack(pady=10)
    status_label = tk.Label(calib_win, text="")
    status_label.pack()
    def start_calib():
        try:
            V_known = float(volt_entry.get())
            freq_known = float(freq_entry.get())
        except ValueError:
        ...
        def update_progress():
            ...
            percent = min(100, (elapsed / CALIBRATION_TIME) * 100)
            progress['value'] = percent
            if elapsed < CALIBRATION_TIME:
                with lock:
                    buf = ring_buffer.copy()
                    ptr = write_ptr
                disp_samples = BUFFER_LEN
                if ptr >= disp_samples:
                    sig = buf[ptr - disp_samples:ptr]
                else:
                    sig = np.concatenate((buf[BUFFER_LEN - (disp_samples - ptr):], buf[:ptr]))
                y = sig * input_sensitivity
                peak = np.max(np.abs(y))
                peaks.append(peak)
                calib_win.after(100, update_progress)
            else:
                if peaks:
                    avg_peak = np.mean(peaks)
                    global AMP_TO_VOLT_RATIO
                    AMP_TO_VOLT_RATIO = V_known / avg_peak
                    status_label.config(text=f"Calibration done. New ratio: {AMP_TO_VOLT_RATIO:.2f}")
                else:
                    status_label.config(text="No data collected")
        update_progress()
    calib_btn = tk.Button(calib_win, text="Calibrate", command=start_calib)
    calib_btn.pack(pady=5)
calib_button.config(command=open_calib_window)

# ==== Metrics ====
def calculate_thd_from_fft(fft_mag, fund_idx):
    if 2 * fund_idx >= len(fft_mag) or fft_mag[fund_idx] < 1e-10:  # Check for invalid index or near-zero amplitude
        return 0.0
