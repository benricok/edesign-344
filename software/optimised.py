...
def update(frame):
    ...
    try:
        ...
        # Redraw only if large scale change
        ...
        if signal_present:
            ...
            if thd_val < 7.0:
                ...
                if meas is not None:
                    ...
                    print(f"Rise: {mean_rise_s:+.7f} | Fall: {mean_fall_s:+.7f} | Skew: {skew_pct:+.7f} | Duty: {mean_duty:+.3f} | Frac_rise: {frac_rise:+.3f}")


                    # classify by fraction of period occupied by rise/fall
                    if mean_duty > 49 and mean_duty < 51: # and abs(skew_pct) > 2:
                        # === Extract harmonics ===
                        if fund_freq > 0:
                            harmonics = []
                            for n in range(2, 6):
